...
public class d{
    ...
    public static void solve(){
        ...
        while(msb-- > 0){
            ...
            // fw.println(msb + " " + rem);
            if(rem >= msb) continue;
            int x = Math.max(0, rem + 1);
            for(int i=x;i<=msb;i++){
                int max = Math.max(i, msb-i);
                int min = Math.min(i, msb-i);
                long cur = 1;
                for(int j=max+1;j<=msb;j++) cur = cur * j;
                for(int j=1;j<=min;j++) cur = cur / j;
                // fw.println(cur);
                ans += cur;
            }
        }
