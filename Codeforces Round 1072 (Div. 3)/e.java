...
public class e{
    ...
    public static void solve(){
        ...
        for(int i=0;i<n-1;i++) b.get(Math.abs(a[i]-a[i+1])).add(i);
        ...
        for(int i=1;i<n-1;i++){
            ...
            for(int j : b.get(i)){
                ...
                // fw.println("");
                int ind = bs(list, j);
                int x = list.get(ind)[0];
                int y = list.get(ind)[1];
                int lef = j - x + 1;
                int rig = y - j;
                // fw.println(j + " " + ind + " " + x + " "+j+" "+y);
                ans -= 1L * lef * rig;
                list.get(ind)[0] = j + 1;
                list.add(ind, new int[]{x,j});
            }
            fw.print(ans + " ");
        }
        fw.println("");
    }
    public static int bs(ArrayList<int[]> list, int val){
