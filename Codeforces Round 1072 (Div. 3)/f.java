...
public class f{
    ...
    public static boolean solve(){
        ...
        boolean a[] = dfs(tree, 1, -1);
        return a[0];
    }
    public static boolean[] dfs(ArrayList<ArrayList<Integer>> tree, int node, int par){
        boolean ans[] = new boolean[3];
        if(node != 1 && tree.get(node).size()==1) return ans;
        int cnt = 0;
        int a[] = new int[3];
        for(int i : tree.get(node)) if(i != par){
            boolean cur[] = dfs(tree, i, node);
            if(cur[2]) a[2]++;
            if(cur[0]) a[0]++;
            cnt++;
        }
        ans[cnt % 3] = true;
        if(a[2] > 0) ans[(cnt+1) % 3] = true;
        if(a[2] > 1) ans[(cnt+2) % 3] = true;
        if(a[0] > 0) ans[(cnt+2) % 3] = true;
        if(a[0] > 1) ans[(cnt+1) % 3] = true;
        // fw.println(node + " "+ ans[2] + " " + ans[0]);
        return ans;
    }
