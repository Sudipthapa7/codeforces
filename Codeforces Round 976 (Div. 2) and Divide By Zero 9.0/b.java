...
public class b {
    ...
    public static void solve(){
        ...
        //fw.println(k);
        long low = 0;
        long high = 2 * (long)1e18;
        while(low<high){
            long mid = low + (high-low)/2;
            long cnt = mid - (long)Math.sqrt(mid);
            if(cnt<k){
                low = mid+1;
            }
            else high = mid;
        }
        long val = (long)Math.sqrt(low);
        if( val*val == low) low--;
        assert low < (long)2 * 1e18;
        fw.println(low);
        System.out.println((long)Math.sqrt(1000000001000000000L));
    }
