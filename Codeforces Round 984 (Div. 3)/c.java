...
public class c {
    ...
    public static void solve(){
        ...
        int n = st.length();
        int s[] = new int[n];
        for(int i=0;i<n;i++) s[i] = st.charAt(i)-'0';

        int q = fr.nextInt();
        Set<Integer> set = new HashSet<>();
        String t = "1100";
        u:for(int i=0;i<n-3;i++){
            for(int j=0;j<4;j++){
                if(s[i+j]!=t.charAt(j)-'0') continue u;
            }
            set.add(i);
        }
        for(int i=0;i<q;i++){
            ...
            if(s[ind]!=val){
                ...
                u:for(int k=0;k<4;k++){
                    if(j+k<0) continue;
                    if(j+k>=n-3) break;
                    if(set.contains(j+k)) set.remove(j+k);
                    else{
                        for(int l=0;l<4;l++){
                            if(s[j+k+l]!=t.charAt(l)-'0') continue u;
                        }
                        set.add(j+k);
                    }
                }
